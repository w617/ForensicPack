...
def hash_file(
    ...
    hashers = {algorithm: _new_hasher(algorithm) for algorithm in algorithms}
    buffer_size = 1 << 22
    ...
    return {algorithm: hashers[algorithm].hexdigest().upper() for algorithm in algorithms}
...
def pre_hash_files(
    ...
    def _hash_one(record: FileRecord) -> tuple[Path, dict[str, str]]:
        def _progress(increment: int) -> None:
            with counter_lock:
                bytes_done_counter[0] += increment
...
def write_manifest(
    ...
    if algorithms:
        ...
        callbacks.emit_progress(
            ProgressEvent(job_id, "manifest", total_bytes, total_bytes, f"Writing manifest {item_path.name}")
        )
